...
static RNWK_ShortAddrType RNWK_ThisNodeAddr =
    RNWK_ADDR_BROADCAST; /* address of this network node */
static RNWK_AppOnRxCallbackType RNWK_AppOnRxCallback =
    NULL; /* notification callback installed by upper layer */

RNWK_ShortAddrType RNWK_GetThisNodeAddr(void) { return RNWK_ThisNodeAddr; }
...
uint8_t RNWK_PutPayload(uint8_t *buf, size_t bufSize, uint8_t payloadSize,
                        RNWK_ShortAddrType dstAddr, RPHY_FlagsType flags)
{
...
uint8_t RNWK_SendACK(RPHY_PacketDesc *rxPacket, RNWK_ShortAddrType saddr)
{
     ...
     */
    addr = RNWK_BUF_GET_SRC_ADDR(
        rxPacket->phyData); /* who should receive the ack? */
    RNWK_BUF_SET_SRC_ADDR(ackPacket.phyData, saddr); /* set source address */
    RNWK_BUF_SET_DST_ADDR(
        ackPacket.phyData,
        addr); /* destination address is from where we got the data */
...
uint8_t RNWK_OnPacketRx(RPHY_PacketDesc *packet)
{
    ...
    if (addr == RNWK_ADDR_BROADCAST ||
        addr == RNWK_GetThisNodeAddr()) { /* it is for me :-) */
        type = RMAC_GetType(packet->phyData,
                            packet->phySize); /* get the type of the message */
        if (RMAC_MSG_TYPE_IS_ACK(type) &&
            RMAC_IsExpectedACK(packet->phyData, packet->phySize)) {
            /* it is an ACK, and the sequence number matches. Mark it with a
             * flag and return, as no need for further processing */
            ...
            if (RNWK_AppOnRxCallback != NULL) {    /* do we have a callback? */
                if (RMAC_MSG_TYPE_REQ_ACK(type)) { /* ACK requested */
                    (void)RNWK_SendACK(
                        packet,
                        RNWK_GetThisNodeAddr()); /* send ack message back */
                }
...
void RNWK_Deinit(void) { /* nothing needed */ }
