...
/* payload format is: <type><size><data> */
...
/* <type> index */
#define RAPP_BUF_IDX_TYPE (RNWK_BUF_IDX_PAYLOAD + 0)
/* <size> index */
#define RAPP_BUF_IDX_SIZE (RNWK_BUF_IDX_PAYLOAD + 1)
/* <app payload> index */
#define RAPP_BUF_IDX_PAYLOAD (RNWK_BUF_IDX_PAYLOAD + 2)

#define RAPP_BUF_TYPE(phy) ((phy)[RAPP_BUF_IDX_TYPE])
#define RAPP_BUF_SIZE(phy) ((phy)[RAPP_BUF_IDX_SIZE])

#define RAPP_BUF_PAYLOAD_START(phy)                                            \
    (RNWK_BUF_PAYLOAD_START(phy) + RAPP_HEADER_SIZE)
...
/* message handler */
typedef uint8_t (*RAPP_MsgHandler)(RAPP_MSG_Type type, uint8_t size,
                                   uint8_t *data, RAPP_ShortAddrType srcAddr,
                                   bool *handled, RAPP_PacketDesc *packet);
